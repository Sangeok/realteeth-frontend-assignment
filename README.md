...
### 기상청 API
- **`WEATHER_DATA_API_KEY`**: 기상청 API 인증키 (URL 인코딩된 값)
  - 발급처: [공공데이터 포털](https://www.data.go.kr) → "기상청_단기예보 ((구)_동네예보) 조회서비스" 신청
- **`WEATHER_DATA_FORECAST_BASE_URL`**: 기상청 단기예보 엔드포인트
  - Endpoint: `https://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getVilageFcst`
- **`WEATHER_DATA_NOWCAST_BASE_URL`**: 기상청 초단기실황 엔드포인트
  - Endpoint: `https://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getUltraSrtNcst`

### Kakao API
- **`KAKAO_REST_API_KEY`**: Kakao 역지오코딩 REST API 키
  - 발급처: [Kakao Developers](https://developers.kakao.com) → 앱 생성 → REST API 키
- **`KAKAO_GEOCODER_BASE_URL`**: Kakao 역지오코딩 엔드포인트
...
### 1. 확장성과 유지보수성을 고려한 FSD (Feature-Sliced Design) 아키텍처 도입
...
### 2. 로딩 UX 안정화 및 장애 격리 (Skeleton + Local ErrorBoundary)
...
### 3. TanStack Query 기반의 효율적인 서버 상태 및 캐싱 관리
...
### 4. 관심사 분리를 통한 클라이언트 상태 관리 (Zustand)
...
### 5. 외부 API 응답을 신뢰하지 않는 방어적 프로그래밍 (Zod 런타임 스키마 검증)
...
### 6. 끊김 없는 UX를 위한 클라이언트 사이드 인메모리 검색 지원
